...
CryptoJS.lib.Cipher || function (n) {
    ...
    for (var i, tt, s = CryptoJS, y = s.lib.BlockCipher, h = s.algo, t = [], p = [], w = [], b = [], k = [], d = [], c = [], l = [], a = [], v = [], u = [], f = 0; 256 > f; f++)u[f] = 128 > f ? f << 1 : f << 1 ^ 283;
    ...
    h = h.AES = y.extend({
        _doReset: function () {
            ...
            for (e = this._invKeySchedule = [], r = 0; r < f; r++)i = f - r, n = r % 4 ? u[i] : u[i - 4], e[r] = 4 > r || 4 >= i ? n : c[t[n >>> 24]] ^ l[t[n >>> 16 & 255]] ^ a[t[n >>> 8 & 255]] ^ v[t[n & 255]]
            ...
            for (var b = this._nRounds, h = n[t] ^ i[0], c = n[t + 1] ^ i[1], l = n[t + 2] ^ i[2], s = n[t + 3] ^ i[3], a = 4, w = 1; w < b; w++)var v = r[h >>> 24] ^ u[c >>> 16 & 255] ^ f[l >>> 8 & 255] ^ e[s & 255] ^ i[a++], y = r[c >>> 24] ^ u[l >>> 16 & 255] ^ f[s >>> 8 & 255] ^ e[h & 255] ^ i[a++], p = r[l >>> 24] ^ u[s >>> 16 & 255] ^ f[h >>> 8 & 255] ^ e[c & 255] ^ i[a++], s = r[s >>> 24] ^ u[h >>> 16 & 255] ^ f[c >>> 8 & 255] ^ e[l & 255] ^ i[a++], h = v, c = y, l = p;
...
aesEncrypt = function (n) {
    var t = CryptoJS.MD5("login.189.cn"), i = CryptoJS.enc.Utf8.parse(t), r = CryptoJS.enc.Utf8.parse("1234567812345678"), u = CryptoJS.AES.encrypt(n, i, {iv: r});
    return u + ""
};
aesDecrypt = function (n) {
    var t = CryptoJS.MD5("login.189.cn"), i = CryptoJS.enc.Utf8.parse(t), r = CryptoJS.enc.Utf8.parse("1234567812345678");
...
valAesEncryptSet = function () {
    ...
    return n == "" && (t = this.aesEncrypt(i)), this.val(t), this.val()
};


createEcsReqinfoLogin1=function () {
    return this.aesEncrypt(this.val())
};

function myEncrypt(parm) {
    return this.aesEncrypt(parm);
}

function createinfoLogin1(param) {
    return this.aesEncrypt(param)
}